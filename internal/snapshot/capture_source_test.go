	...
	for _, failedPass := range []int{1, 2} {
		for position, boundary := range sequence {
			for _, failure := range []struct {
				...
				{name: "git-read-limit", cause: git.ErrReadLimit, limit: true},
				{name: "git-read-wrapped-limit", cause: fmt.Errorf("native read: %w", git.ErrReadLimit), limit: true},
				{name: "git-read-limit-text", cause: errors.New(git.ErrReadLimit.Error())},
				{name: "git-read-cancel-and-limit", cause: git.ErrReadLimit, cancel: true, limit: true},
				{name: "index-preflight-limit", cause: git.ErrIndexPreflightLimit, limit: true},
				{name: "index-preflight-wrapped-limit", cause: fmt.Errorf("preflight: %w", git.ErrIndexPreflightLimit), limit: true},
				{name: "index-preflight-limit-text", cause: errors.New(git.ErrIndexPreflightLimit.Error())},
				{name: "index-preflight-cancel-and-limit", cause: git.ErrIndexPreflightLimit, cancel: true, limit: true},
